...
setfg (RenderState dpy _ w gc) = setForeground dpy gc
setbg (RenderState dpy _ w gc) = setBackground dpy gc
...
zTitleWidget global@(config, rs, wrs, ch) z =
  zAddThreadFilter (thr, val) global z >>= zDzenDisplayFilter global where
    ...
    simpleThread = zStatelessThread (parseLine <$> getLine)
...
readBatteryString x = (head . lines) <$> readFileWithFallback ("/sys/class/power_supply/BAT1/" ++ x)
